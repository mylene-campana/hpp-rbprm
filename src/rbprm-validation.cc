...
namespace hpp {
  ...
  namespace rbprm {
    ...
    (const model::RbPrmDevicePtr_t& robot,
     ...
     std::size_t nbFilterMatch)
    {
      hppDout (info, "create RbprmValidation");
      if (nbFilterMatch == 0)
	nbFilterMatch = filter.size ();
      hppDout (info, "nbFilterMatch= " << nbFilterMatch);
      RbPrmValidation* ptr = new RbPrmValidation (robot, filter, affFilters, affordances, geometries, nbFilterMatch);
      return RbPrmValidationPtr_t (ptr);
    }

    RbPrmValidation::RbPrmValidation (const model::RbPrmDevicePtr_t& robot, const std::vector<std::string>& filter, const std::map<std::string, std::vector<std::string> >& affFilters, const std::map<std::string, std::vector<model::CollisionObjectPtr_t> >& affordances, const core::ObjectVector_t& geometries, const std::size_t nbFilterMatch)
